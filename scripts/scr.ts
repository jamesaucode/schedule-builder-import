...
const findFirstFinal = (strippedArray: string[]) => {
    ...
    for (let i = 0; i < strippedArray.length; i++) {
        ...
        if (finalExam !== null) {
            const finalExamStartDate = getFinalExamDates(finalExam)[0].slice(
                0,
                10,
            );
...
const createEventObjects = () => {
    ...
    let userInputDate: string =
        CURRENTYEAR + '-' + datePickNode.value.toString();
    ...
    for (let i = 0; i < strippedArray.length; i++) {
        ...
        const finalExam: RegExpExecArray = getFinalExamMatches(
            strippedArray[i],
        );
        if (finalExam !== null) {
